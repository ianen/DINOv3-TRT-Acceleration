...
| 模式                                | Token 序列长度 | 形状               | 约束                                 |
| ----------------------------------- | -------------- | ------------------ | ------------------------------------ |
| **API 默认（裁剪 register）** | 197            | `[B, 197, 1024]` | 本项目主路径                         |
...
> **来源说明**：4/12/16/20 这一组合**非 DINOv3 官方 canonical 推荐**，源自 DPT 风格密集预测任务的社区惯例（24 层 ViT-L 上常见 [5,11,17,23]，本项目选择偏中后层组合 [4,12,16,20]）。该约束自动排除 ViT-S/B（仅 12 层）。层组合的 ablation 已纳入 stretch goal。

### 2.2 技术栈版本约束

| 维度           | 选型                                                                                                                                       | 说明                                                                                        |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------- |
| 模型权重       | DINOv3 ViT-L/16 LVD-1689M（safetensors 格式）；HuggingFace `facebook/dinov3-vitl16-pretrain-lvd1689m`（需点击同意 license）              | 自定义 license / 渠道 / 格式                                                                |
...
| C++ binding    | pybind11                                                                                                                                   | —                                                                                          |
| 量化框架       | **NVIDIA TensorRT Model Optimizer（Q/DQ 显式量化）为主路径**；IInt8EntropyCalibrator2 已 deprecated（TRT 10.1+），仅作 baseline 对照 | 官方当前推荐路径                                                                            |
...
> `src/export/rope_patch.py` 集中处理 DINOv3 RoPE 源码改造（去除 If 条件分支）；`src/quantize/` 双路径（ModelOpt 主 + Entropy baseline）。
...
| 理由           | API 默认裁剪 register 与既定形状契约一致；wrapper 显式 `return tuple` 让 ONNX 识别 4 个输出；不侵入第三方源（除 RoPE 必要改造，见 ADR-007）             |
...
| 理由           | `IInt8EntropyCalibrator2` 自 TRT 10.1 起已 deprecated；ModelOpt 显式 Q/DQ 是 NVIDIA 官方当前推荐路径，支持 per-channel + per-tensor 混合、敏感层混合精度回退                                              |
...
| 影响           | `utils/preprocess.py` 与 C++ 等价实现共享同一组归一化常量；后续可迁至 NPP/CV-CUDA |
...
def build_engine(
    ...
    legacy_calibrator: Optional[IInt8Calibrator] = None, # baseline 对照
    workspace_gb: int = 4,
...
| `latency_p50_std_ms`        | float   | 跨 1000 次的 std，门禁 ≤ 5%                   |
| `latency_p95_ms`            | float   |                                                |
| `latency_p99_ms`            | float   |                                                |
| `latency_median_trimmed_ms` | float   | 中位数去极值（剔除最高 5% + 最低 5%）          |
| `throughput_imgs_s`         | float   |                                                |
| `gpu_mem_mb`                | int     | nvml 峰值                                      |
| `cos_sim_layer4`            | float   | 4 输出**逐层独立**汇报（不允许仅给均值） |
...
| **确定性数值**       | TRT builder `kOBEY_PRECISION_CONSTRAINTS` + `kSTRICT_TYPES` + **`kDISABLE_TF32`**；C++ 端 `CUBLAS_PEDANTIC_MATH`                                                                     |
...
| **P2** | ONNX 导出 & 验证         | 4 天 | P1 完成         | 4 输出 ONNX 通过 ORT 与 PyTorch 对齐（cos ≥ 0.9999）；RoPE If 节点已消除                   |
| **P3** | TRT FP32/FP16 引擎构建   | 5 天 | P2 ONNX 可用    | FP16 端到端加速 ≥ 1.5×（stretch 1.8×）；逐层 cos ≥ 0.999                                |
| **P4** | INT8 PTQ 校准 & 精度调优 | 6 天 | P3 FP16 通过    | INT8 引擎逐输出 cos ≥ 0.99；下游精度跌幅 ≤ 1%（ModelOpt 主路径 + Entropy baseline 对照）  |
| **P5** | Benchmark 全矩阵 & 报告  | 4 天 | P3+P4 引擎齐全  | 完整 benchmark CSV（含 std + trimmed median）；瓶颈定位明确                                 |
| **P6** | C++ 生产封装             | 5 天 | P5 最优引擎确定 | C++ 与 Python 数值一致（按精度分档）；C++ 吞吐 ≥ Python 1.1×；零内存泄漏                  |
...
| **Portability**           | 主支持**sm_120（Blackwell，主力机 RTX 5080）**；兼容 sm_86 / sm_89 / sm_90；CUDA ≥ 12.8 / TRT ≥ 10.13；Linux 主、Windows 次（Linux WSL2 作为 sm_120 备选） | CI 矩阵构建；主力机基线必跑                                | `engine/` · `cpp/`                 |
